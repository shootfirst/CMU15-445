...
LRUKReplacer::~LRUKReplacer() {
    for (auto list_it = first_time_.begin(); list_it != first_time_.end(); list_it++) delete *list_it;
    for (auto tree_it = k_time_.begin(); tree_it != k_time_.end(); tree_it++) delete tree_it->second;
}
...
auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool { 
    ...
    for (auto tree_it = k_time_.begin(); tree_it != k_time_.end(); tree_it++) {
        LRU_K_Info *info = tree_it->second;
        if (info->evictable_) {
            *frame_id = info->frame_id_;
...
void LRUKReplacer::RecordAccess(frame_id_t frame_id) {
    ...
    if (tree_map_it != nullptr) {
        LRU_K_Info *info = tree_map_it->second->second;
        // delete old last kth timestap
        info->timestap_list_.pop_front();
        // add new visited time_stap
        info->timestap_list_.push_back(current_timestamp_++);

        // we need update last kth timestap, so we firwst delete it, and add it with new last kth timestap (这里顺序不能倒换)
    ...
    if (replacer_size_ == all_cnt_) return;
    LRU_K_Info *new_info = new LRU_K_Info(frame_id, false);
    all_cnt_++;
